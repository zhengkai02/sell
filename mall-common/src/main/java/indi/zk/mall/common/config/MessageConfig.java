package indi.zk.mall.common.config;
...
public class MessageConfig {
     ...
     * 车企订单支付回调队列
     */
    @Bean("orderPayCallback")
    public Queue orderPayCallback() {
        return new Queue(MessageDef.ORDER_PAY_CALLBACK_QUEUE);
    }

    /**
     * 使用路由键（ORDER_PAY_CALLBACK_TOPIC）把队列（Queue）绑定到交换器（Exchange）
     * @param queueMessage
     * @param exchange
     * @return
     */
    @Bean
    public Binding bindingExchangeOrderPayCallback(@Qualifier("orderPayCallback") Queue queueMessage, TopicExchange exchange) {
        return BindingBuilder.bind(queueMessage).to(exchange).with(MessageDef.ORDER_PAY_CALLBACK_TOPIC);
    }
     ...
     */
    @Bean("flowOpenNotify")
    public Queue flowOpenNotify(){
        return new Queue(MessageDef.FLOW_OPEN_NOTIFY_QUEUE);
    }

    /**
     * 使用路由键（FLOW_OPEN_NOTIFY_TOPIC）把队列（Queue）绑定到交换器（Exchange）
     * @param queueMessage
     * @param exchange
     * @return
     */
    @Bean
    public Binding bindingExchangeFlowOpenNotify(@Qualifier("flowOpenNotify") Queue queueMessage, TopicExchange exchange){
        return BindingBuilder.bind(queueMessage).to(exchange).with(MessageDef.FLOW_OPEN_NOTIFY_TOPIC);
    }
